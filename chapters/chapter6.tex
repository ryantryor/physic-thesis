...
\begin{enumerate}
  \item $\xi(s)$ 上核函数性能形成明确层级：
    ...
    条件下不显著，暂不推荐基于 $\xi(\mu)$ 的核函数选择；本文核函数推荐结论仅基于 $\xi(s)$
    上的 MAE 比较。
  ...
  \item $\xi(s)$ 仿真器精度（MAE/$\sigma_{\rm stat}=3.4\%$）满足精密宇宙学要求；
    ...
    和低信噪比。
  \item 在当前协方差设定下，联合约束 FoM 排序为：Mat\'ern$(5/2)$ ($58.9$) ${}>{}$
    Mat\'ern$(3/2)$ ($58.3$) ${}>{}$ RQ ($57.7$) ${}>{}$ Mat\'ern$(1/2)$ ($56.3$)
    ${}>{}$ Combined ($50.4$) ${}>{}$ RBF ($49.3$)；Mat\'ern$(5/2)$ 的联合 FoM 相比
    $\xi(s)$ 单独提升约 $25\%$，表明 $\xi(\mu)$ 包含互补信息。\textbf{FoM 绝对值因
    协方差矩阵高估和箱体不匹配不具物理意义，仅作核函数相对排序；$\xi(s)$ 上的 LOO MAE
    是核函数推荐最可靠的依据，$\xi(\mu)$ 的核函数排序不作为推荐依据。}

...
\begin{abstract}
  ...
  选择。$\xi(\mu)$ 上核函数差异在无泄露条件下不显著，不作为核函数推荐依据。
  Mat\'ern$(\nu=5/2)$ 核的联合约束品质因子 ${\rm FoM}=58.9$（bootstrap 标准差 $0.2$），
  在相同协方差设定下相比单独使用 $\xi(s)$ 提升约 $25\%$（FoM 的绝对数值因协方差矩阵高估
  不具物理意义，仅作核函数相对排序）。六种核的联合 FoM 从高到低依次为：
  Mat\'ern$(5/2)$ ($58.9$) ${}>{}$ Mat\'ern$(3/2)$
...
\begin{abstract*}
  ...
  leakage-free conditions and are \emph{not} used as a basis for kernel recommendation.
  (3) Joint FoM ranking (the absolute FoM values carry no direct physical meaning due
  to covariance-matrix overestimation and are used only for relative kernel ordering):
  Mat\'ern$(5/2)$ ($58.9$) ${}>{}$ Mat\'ern$(3/2)$ ($58.3$) ${}>{}$ RQ ($57.7$) ${}>{}$
  Mat\'ern$(1/2)$ ($56.3$) ${}>{}$ Combined ($50.4$) ${}>{}$ RBF ($49.3$).
